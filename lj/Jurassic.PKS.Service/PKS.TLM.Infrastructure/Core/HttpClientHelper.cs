...
namespace PKS.TLM.Infrastructure.Core
{
    ...
    public class HttpClientHelper
    {
        ...
        /// <summary>构造函数</summary>
        public HttpClientHelper(bool useDefault = true)
        {
        ...
        private string GetQueryUrl(string url, Dictionary<string, object> queryParams)
        {
        ...
        {
            var url2 = GetQueryUrl(url, queryParams);
        ...
        private StreamContent BuildStreamContent(string fileName, Stream stream)
        {
            ...
            streamContent.UseContentDisposition(fileName);
        ...
        /// <summary>上传文件</summary>
        public T Upload<T>(string url, string fileName, Stream stream)
        {
            return UploadAsync<T>(url, fileName, stream).Result;
        }
        /// <summary>上传文件</summary>
        public async Task<T> UploadAsync<T>(string url, string fileName, Stream stream)
        {
            var httpContent = BuildMultipartFormDataContent(fileName, stream);
            var response = await SendAsync(HttpMethod.Post, url, httpContent).ConfigureAwait(false);
            return await ReadContentAsync<T>(response);
